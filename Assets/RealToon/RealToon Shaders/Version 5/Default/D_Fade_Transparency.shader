    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR 
			{
				...
				half3 RTD_ST_OFF_OTHERS = (RTD_RL_CHE_1*RTD_SON_CHE_1*lightColor.rgb);
					...
					half RTD_ST_LFAST_OO = lerp(lerp( RTD_NDOTL, (attenuation*RTD_NDOTL), _LightFalloffAffectShadowT ) , 1 , _STIL ); //5.0.4
				...
				half3 RTD_R_OFF_OTHERS = ( lerp( RTD_TEX_COL , _MainTex_var.rgb , _MCIALO)  * lerp((RTD_LAS*RTD_LVLC*RTD_ST_IS),RTD_ST,RTD_SS)); //5.0.4
					...
					half3 RTD_R_SEL = lerp(RTD_R,lerp(RTD_R,RTD_TEX_COL,_MaskSelfLit_var.r),_SelfLitIntensity);
                ...
                fixed4 finalRGBA = fixed4(finalColor,RTD_TRAN_OPA_Sli);
				UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR 
			{
				...
				half3 RTD_CA_OFF =  ((RTD_MCIALO*RTD_SL*RTD_STIAL)+RTD_RL)*lightfos;
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR 
			{
				...
				float dither = tex3D(_DitherMaskLOD, float3(i.pos.xy * 0.25, RTD_TRAN_OPA_Sli * 0.99)).a;
